import { Plugin } from "@macrograph/core";
...
import { Layer, Queue, Tracer } from "effect";
...
export default class ProjectEditor extends Cloudflare.DurableObjectNamespace<ProjectEditor>()(
	...
	Effect.gen(function* () {
		...
		return Effect.gen(function* () {
			...
			yield* editor.plugin(CorePlugin);

			const RpcLayer = Layer.mergeAll(RpcSerialization.layerJsonRpc(), EditorRpc.handlerLayer);

			const rpcWs = yield* makeRpcServerHttpEffectWebsocket(EditorRpc.EditorRpcs).pipe(
				Effect.provide(RpcLayer),
			);

			const rpcHttp = yield* RpcServer.toHttpEffect(EditorRpc.EditorRpcs).pipe(
				Effect.provide(RpcLayer),
			);
