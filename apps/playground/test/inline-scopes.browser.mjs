...
const node = (id, pkg, schema, x, y, properties = {}, inputDefaults = {}) => ({
  id,
  name: id === "match" ? "Match Enum" : id === "print" ? "Print" : "Tick",
...
const pin = (page, node, direction, id) =>
  page.locator(
    `[data-node-id=${JSON.stringify(node)}][data-io-direction=${JSON.stringify(direction)}][data-io-id=${JSON.stringify(id)}]`,
  );
...
export async function inlineScopes(context) {
  ...
  try {
    ...
    const inputPositions = () =>
      page.locator('[data-graph-node-id="match"] [data-io-direction="input"]').evaluateAll((pins) =>
        pins.map((pin) => {
    ...
    assert.equal(
      await page
        .locator('[data-graph-node-id="match"] [data-scope-group]')
        .first()
    ...
    assert.deepEqual(
      project.graphs.demo.connections
        ...
        .map((wire) => wire.outIo),
      [
        { _tag: "ScopeExec", scope },
        { _tag: "ScopeField", scope, field },
      ],
    );
    // Each scope is one bordered wrapper in the independent output column.
    const secondScope = 'variant:"Variant 2"';
    await pin(page, "match", "output", JSON.stringify(["Port", secondScope])).click({
      button: "right",
    });
    await page.getByRole("button", { name: "Split scope", exact: true }).click();
    const secondExec = () =>
      pin(page, "match", "output", JSON.stringify(["ScopeExec", secondScope]));
    ...
    const borders = await page
      ...
      .evaluateAll((elements) =>
        elements.map((element) => {
          ...
          return {
            tag: element.tagName,
            rows: element.querySelectorAll(':scope > [data-io-row="output"]').length,
            position: style.position,
            scope: element.getAttribute("data-scope-group"),
            ...
            radiusTop: style.borderTopLeftRadius,
    ...
    assert.deepEqual(
      await inputPositions(),
      initialInputs,
      "Output groups must not move input pins",
    );
    ...
    assert.deepEqual(
      await columns.evaluateAll((columns) =>
        columns.map((column) =>
          [...column.querySelectorAll("[data-io-row]")].map(
            (row) => row.querySelectorAll("[data-io-direction]").length,
          ),
        ),
      ),
      [
    ...
    await page.waitForFunction(
      (ids) => {
        const starts = [
          ...document.querySelectorAll('[data-active-graph-canvas] svg path[stroke-width="2"]'),
        ].map((path) => {
        ...
        return ids.every((id) => {
          const pin = [
            ...document.querySelectorAll('[data-node-id="match"][data-io-direction="output"]'),
          ].find((element) => element.getAttribute("data-io-id") === id);
          if (!pin) return false;
          const bounds = pin.getBoundingClientRect();
          return starts.some(
            (point) =>
              Math.abs(point.x - (bounds.x + bounds.width / 2)) < 1 &&
              Math.abs(point.y - (bounds.y + bounds.height / 2)) < 1,
          );
        });
      },
    ...
    assert.equal(
      await page.getByRole("button", { name: "Bundle scope", exact: true }).isDisabled(),
      true,
    );
...
export async function singleSchemaDrop(context) {
  ...
  await page.evaluate(() => {
    ...
    new MutationObserver(() => {
      if (document.querySelector('[role="dialog"][aria-label="Create node"]'))
        window.schemaMenuOpened = true;
