...
import { For, Show, action, createSignal, onSettled, untrack, type Component } from "solid-js";
...
export const RpcMethod: Component<MethodProps> = (props) => {
  ...
  const [payloadText, setPayloadText] = createSignal(
    untrack(() => {
      if (SchemaAST.isVoid(props.rpc.payloadSchema.ast)) return "";
      try {
        return JSON.stringify(generateJsonTemplate(props.rpc.payloadSchema.ast), null, 2);
      } catch {
        return "{}";
      }
    }),
  ...
  return (
    <div class="border border-gray-5 rounded-lg overflow-hidden">
      <button
        ...
        class="w-full flex items-center gap-3 px-4 py-3 bg-gray-2 hover:bg-gray-3 text-left transition-colors"
      >
        <span class="text-gray-11 font-mono text-sm">{expanded() ? "▼" : "▶"}</span>
        <span class="font-mono font-semibold text-gray-12">{props.rpc._tag}</span>
        <Show when={isStreaming()}>
          <span class="rounded-full bg-purple-400/15 px-2 py-0.5 text-xs font-medium text-purple-300">
            Stream
          </span>
        </Show>
      </button>
      ...
      <Show when={expanded()}>
        <div class="border-t border-gray-5">
          <div class="grid grid-cols-[auto_1fr] gap-x-4 gap-y-3 px-4 py-3 text-sm">
            <span class="text-gray-11 font-medium">Payload</span>
            <div>
              <Show when={hasPayload()} fallback={<span class="text-gray-11">void</span>}>
                <SchemaView schema={props.rpc.payloadSchema} />
              </Show>
            </div>

            <Show when={isStreaming()}>
              <span class="text-gray-11 font-medium">Events</span>
              <div>
            ...
            <Show when={!isStreaming()}>
              <span class="text-gray-11 font-medium">Response</span>
              <div>
                <Show when={!successIsVoid()} fallback={<span class="text-gray-11">void</span>}>
            ...
            <Show when={!errorIsNever()}>
              <span class="text-gray-11 font-medium">Errors</span>
              <div>
          ...
          <div class="border-t border-gray-5 px-4 py-3 space-y-3">
            <button
              onClick={generateTemplate}
              class="text-xs text-gray-11 hover:text-gray-12 underline underline-offset-2"
            >
              Fill template
            </button>
            ...
            <Show when={hasPayload()}>
              <div class="relative">
                <textarea
                  ...
                  onInput={handleInput}
                  rows={6}
                  class="mt-2 w-full rounded border border-gray-5 bg-gray-2 px-3 py-2 font-mono text-xs text-gray-12 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-blue-400"
                  placeholder='{ "key": "value" }'
                />
                <Show when={showAutocomplete()}>
                  <div class="absolute z-10 left-0 right-0 mt-1 bg-gray-1 border border-gray-5 rounded shadow-lg max-h-40 overflow-y-auto">
                    <For each={autocompleteOptions()}>
                      {(opt) => (
                        <button
                          ...
                          class="block w-full px-3 py-1.5 text-left text-xs text-gray-12 hover:bg-blue-500/15"
                        >
                          {opt.label}
                        </button>
                      )}
            ...
            <div class="flex items-center gap-3">
              ...
              <Show when={events().length > 0}>
                <span class="rounded bg-purple-400/15 px-2 py-0.5 text-xs text-purple-300">
            ...
            <Show when={events().length > 0}>
              <div class="border border-gray-5 rounded-lg divide-y divide-gray-4 max-h-80 overflow-y-auto">
                <For each={events()}>
                  {(event, i) => (
                    <div class="px-3 py-2">
                      <span class="text-xs text-gray-11 font-mono mr-2">#{i() + 1}</span>
                      <pre class="inline text-xs font-mono text-gray-12">
            ...
            <Show when={result()}>
              <pre class="p-3 bg-gray-2 border border-gray-5 rounded-lg text-xs font-mono overflow-x-auto">
                {JSON.stringify(result()!.value, null, 2)}
              </pre>
            </Show>

            <Show when={errorText()}>
              <pre class="overflow-x-auto rounded-lg border border-red-7 bg-red-3 p-3 font-mono text-xs text-red-11">
                {errorText()}
              </pre>
            </Show>
          </div>
