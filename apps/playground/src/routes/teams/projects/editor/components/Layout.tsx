              ...
              {(tab) => {
                ...
                return (
                  ...
                  >
                    <button
                      type="button"
                      class="focus-ring flex h-full flex-row items-center text-nowrap px-3 text-xs font-medium [@media(pointer:coarse)]:pr-9"
                      onClick={() => props.onSelect(tab.id)}
                    >
                      <span
                        class={`transition-colors ${
                          selected() ? "text-gray-12" : "text-gray-10 group-hover/tab:text-gray-12"
                        }`}
                      >
                        {tab.title}
                      </span>
                      <Show when={tab.description}>
                        <span class="ml-1 text-[10px] font-normal text-gray-10">
                          {tab.description}
                        </span>
                      </Show>
                    </button>
                    <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center opacity-0 group-hover/tab:opacity-100 focus-within:opacity-100 [@media(pointer:coarse)]:opacity-100">
                      <span
                      ...
                      >
                        <button
                          ...
                          class="focus-ring pointer-events-auto rounded-sm bg-transparent p-0.5 text-gray-11 hover:bg-gray-6 hover:text-gray-12"
        ...
        <div class="flex h-full shrink-0 flex-row items-center gap-1 border-b border-l border-gray-5 px-2">
          <Show when={props.onSplit}>
            <button
              type="button"
              title="Split Pane"
              class="focus-ring flex size-5 items-center justify-center rounded-sm bg-transparent text-gray-11 opacity-0 hover:bg-gray-6 hover:text-gray-12 group-hover:opacity-100 [@media(pointer:coarse)]:opacity-100"
