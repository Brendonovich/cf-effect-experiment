...
export const make = (deploymentsResource: Cloudflare.R2.Bucket) =>
  Effect.gen(function* () {
    ...
    const queueScope = Effect.fnUntraced(function* (projectId: string, queueId: string) {
      const deployment = yield* loadDesiredDeployment(projectId).pipe(
        Effect.catchCause((cause) =>
          Effect.fail(
            new Queue.OperationError({
              queueId,
    ...
    return {
      ...
      queueSnapshot: (projectId: string) =>
        queueScope(projectId, "").pipe(
          Effect.flatMap((scope) => projectIngressDos.getByName(projectId).queueSnapshot(scope)),
          Effect.catch(() => Effect.succeed([])),
        ),
      queuePause: (projectId: string, queueId: string, paused: boolean) =>
        queueScope(projectId, queueId).pipe(
          Effect.flatMap((scope) =>
            projectIngressDos.getByName(projectId).queuePause(scope, queueId, paused),
          ),
        ),
      queueAdvance: (projectId: string, queueId: string) =>
        queueScope(projectId, queueId).pipe(
          Effect.flatMap((scope) =>
            projectIngressDos.getByName(projectId).queueAdvance(scope, queueId),
          ),
        ),
      queueRemove: (projectId: string, queueId: string, itemId: string) =>
        queueScope(projectId, queueId).pipe(
          Effect.flatMap((scope) =>
            projectIngressDos.getByName(projectId).queueRemove(scope, queueId, itemId),
          ),
        ),
      queueClear: (projectId: string, queueId: string) =>
        queueScope(projectId, queueId).pipe(
          Effect.flatMap((scope) =>
            projectIngressDos.getByName(projectId).queueClear(scope, queueId),
          ),
        ),
    };
  });
