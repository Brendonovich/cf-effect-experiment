...
export const projectIngressImplementation = Effect.gen(function* () {
  ...
  const functionQueues = yield* ProjectFunctionQueues.make({
    ...
    send: (delivery) =>
      Effect.tryPromise({
        try: () =>
          FunctionQueueProtocol.queueBinding(workerEnvironment?.FunctionWorkQueue).send(delivery),
        catch: (cause) => cause,
      }),
    workflows: {
      create: (options) =>
        FunctionQueueProtocol.workflowBinding(workerEnvironment?.FunctionExecutionWorkflow).create(
          options,
        ),
      get: (id) =>
        FunctionQueueProtocol.workflowBinding(workerEnvironment?.FunctionExecutionWorkflow).get(id),
    },
  });
  ...
  return Effect.gen(function* () {
    ...
    const deploy = Effect.fn("ProjectIngressDO.deploy")(function* (request: DeployRequest) {
      ...
      yield* functionQueues.configure(
        {
          projectId: request.projectId,
          deploymentId: request.deploymentId,
          r2Key: request.r2Key,
    ...
    const undeploy = Effect.fn("ProjectIngressDO.undeploy")(function* () {
      ...
      if (Option.isSome(previous) && previous.value.projectId !== undefined)
        yield* functionQueues.stop({
          projectId: previous.value.projectId,
          deploymentId: previous.value.deploymentId,
          r2Key: previous.value.r2Key,
        });
    ...
    const alarm = Effect.gen(function* () {
      ...
      if (events.some((event) => event.id === ProjectFunctionQueues.alarmId))
        yield* functionQueues.reconcile;
...
export type ProjectIngressShape = Effect.Success<
  Effect.Success<typeof projectIngressImplementation>
>;
