...
- `Executor.Service.invokeFunction(functionId, inputs, { executionPath,
...
resolves the current database deployment through `loadDesiredDeployment`; the
state stream polls every two seconds and suppresses unchanged snapshots. Preview/local queue controls
...
Run `pnpm typecheck` at the repository root. Unit tests use fake Queue/Workflow
bindings and do not deploy resources.
