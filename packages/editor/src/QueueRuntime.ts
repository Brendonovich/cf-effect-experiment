...
export const layer = Layer.effectContext(
  Effect.gen(function* () {
    ...
    const updates = yield* PubSub.sliding<ReadonlyArray<Queue.State>>({ capacity: 1, replay: 1 });
    ...
    return Context.make(Service, {
      ...
      Context.add(Mount, {
        set: (next) =>
          Effect.gen(function* () {
            runtime = next;
            yield* next.snapshot.pipe(Effect.flatMap((states) => PubSub.publish(updates, states)));
            yield* next.changes.pipe(
              Stream.runForEach((states) => PubSub.publish(updates, states)),
              Effect.forkIn(scope),
            );
          }),
      }),
