...
export namespace EventSubSocket {
  ...
  export const make = Effect.gen(function* () {
    ...
    const getEvent = yield* Stream.never.pipe(
      Stream.pipeThroughChannel(Socket.makeWebSocketChannel("wss://eventsub.wss.twitch.tv/ws")),
      Stream.decodeText(),
      Stream.mapEffect((s) => S.decodeEffect(S.fromJsonString(EventSubMessage.EventSubMessage))(s)),
    ...
    yield* Effect.logInfo("Twitch EventSub WebSocket session welcomed", {
      sessionId: firstEvent.payload.session.id,
    });
    return { id: firstEvent.payload.session.id, stream: Stream.fromPull(Effect.succeed(getEvent)) };
  });
}

export namespace EventSubMessage {
  function makeMessageSchema<TType extends string, TPayload extends S.Struct.Fields>(value: {
    message_type: TType;
    payload: TPayload;
  }) {
    return S.Struct({
...
const SubscriptionType =
  ...
  >(opts: {
    ...
    condition: Condition;
    event: Event;
  }): S.Class<Self, S.TaggedStruct<Type, Event>, {}> & {
    type: Type;
    condition: Condition;
    version: number | string;
  } =>
    Object.assign(S.TaggedClass<Self>()(opts.type, opts.event), opts) as unknown as S.Class<
      Self,
      S.TaggedStruct<Type, Event>,
      {}
    > & {
      type: Type;
      condition: Condition;
...
const UTCTime = S.Union([S.DateTimeUtc, S.DateTimeUtcFromString]);

export const buildCondition = (
  def: { condition: S.Struct.Fields },
  accountId: string,
): Record<string, string> => {
  return Object.fromEntries(Object.keys(def.condition).map((key) => [key, accountId]));
};
