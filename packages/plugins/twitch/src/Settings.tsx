...
const Settings: Component<SettingsProps> = (props) => {
  ...
  const [collapsedAccounts, setCollapsedAccounts] = createSignal<ReadonlyArray<AccountId>>(
    props.state.accounts.map((account) => account.id),
  );
      ...
      <div>
        <For each={props.state.accounts}>
          {(account) => {
            ...
            return (
              <section class="grid grid-cols-[1rem_minmax(0,1fr)_auto] items-center gap-x-3 gap-y-1 border-b border-gray-6 py-3 first:pt-0 last:border-b-0">
                <button
                  type="button"
                  class="focus-ring col-start-1 row-start-1"
                  aria-label={
                    collapsedAccounts().includes(account.id) ? "Expand account" : "Collapse account"
                  }
                  onClick={() =>
                ...
                >
                  <svg
                    viewBox="0 0 16 16"
                    class={`size-4 transition-transform ${collapsedAccounts().includes(account.id) ? "-rotate-90" : ""}`}
                    aria-hidden="true"
                  >
                    <path
                      d="m3 6 5 5 5-5"
                ...
                <h3 class="col-start-2 row-start-1 truncate text-sm font-semibold">
                  {account.displayName}
                </h3>
                <span
                  class={`col-start-1 row-start-2 size-2.5 justify-self-center rounded-full ${connected() ? "bg-green-500" : "bg-red-500"}`}
                />
                ...
                <button
                  class="focus-ring col-start-3 row-start-1 px-1 text-xs font-medium hover:text-gray-11 disabled:opacity-40"
