...
export const WebhookConfiguration = Schema.Struct({
...
const decodeDelivery = Schema.decodeUnknownEffect(
	Schema.fromJsonString(Delivery),
);
	...
	Effect.succeed(
		Effect.succeed({
			handle: Effect.fnUntraced(function* (request) {
				...
				if (
					delivery.value.verification_token !==
					request.configuration.verificationToken
				)
					return { status: 403 };
				...
				const event: Payment = {
					_tag: type,
					webhookId: request.endpoint.metadata.webhookId,
					...fields,
				};
				return {
					status: 200,
