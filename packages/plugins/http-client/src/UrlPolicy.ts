...
export class Rejected extends Schema.TaggedError<Rejected>()("HttpUrlRejected", {
...
>()("@macrograph/plugin-http-client/UrlPolicy") {}
...
const ipv6Segments = (host: string): ReadonlyArray<number> | undefined => {
...
const publicIpv6 = (host: string): boolean => {
  ...
  // Reject transition and special-use ranges that can embed otherwise-blocked IPv4 destinations.
  if (first === 0x2002) return false;
  if (first === 0x2001 && (second < 0x0200 || second === 0x0db8)) return false;
  if (first === 0x3ffe || (first === 0x3fff && (second & 0xf000) === 0)) return false;
  return true;
};
...
export const make = (check: (url: URL) => Effect.Effect<void, Rejected>) =>
  Layer.succeed(
    Service,
    Service.of({ check: (url) => commonCheck(url).pipe(Effect.andThen(check(url))) }),
  );

export const secureLayer = make((url) => {
  if (url.protocol !== "https:") return reject("Production HTTP requests must use HTTPS");
  if (url.port !== "") return reject("Production HTTPS requests must use port 443");
  ...
  // Hostname checks cannot prevent DNS rebinding; deployments must also restrict network egress.
  if (host.includes(":"))
