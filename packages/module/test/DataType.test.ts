...
describe("custom data types", () => {
  it("encodes empty wildcard container defaults without permitting unresolved runtime values", () => {
    const wildcard = DataType.Wildcard("T");
    expect(
      Schema.encodeUnknownSync(DataType.JsonDefaultSchema(DataType.List(wildcard)))([]),
    ).toEqual([]);
    expect(
      Schema.encodeUnknownSync(DataType.JsonDefaultSchema(DataType.Option(wildcard)))(
        Option.none(),
      ),
    ).toEqual({ _tag: "None" });
    expect(() => Schema.encodeUnknownSync(DataType.JsonDefaultSchema(wildcard))("value")).toThrow();
    expect(() =>
      Schema.encodeUnknownSync(DataType.JsonDefaultSchema(DataType.List(wildcard)))(["value"]),
    ).toThrow();
    expect(() =>
      Schema.encodeUnknownSync(DataType.JsonDefaultSchema(DataType.Option(wildcard)))(
        Option.some("value"),
    ...
    expect(DataType.isValue(DataType.List(wildcard), [])).toBe(false);
  });
  it("decodes nested nominal and wildcard descriptors", () => {
    const decode = Schema.decodeUnknownSync(DataType.Descriptor);
    ...
    expect(decode(DataType.List(DataType.Wildcard("T")))).toEqual(
      DataType.List(DataType.Wildcard("T")),
    );
  ...
  it("reports cyclic and excessively deep payloads as schema errors, not recursion defects", () => {
    ...
    expect(
      Schema.decodeUnknownResult(DataType.JsonValueSchema(DataType.Custom(treeId), definitions))(
        cyclic,
      )._tag,
    ).toBe("Failure");
