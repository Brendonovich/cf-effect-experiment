...
interface DiscoveryOptions {
  ...
  readonly modulePath: (manifest: ModulePackage) => string | undefined;
...
const exportPath = (manifest: ModulePackage, name: string) => {
  const value = manifest.exports?.[name];
  return typeof value === "string" ? value : undefined;
};

const manifestPath = (manifest: ModulePackage, name: string) => {
...
const workspaceModules = (options: DiscoveryOptions) => {
  ...
  const modulesDirectory = resolve(workspaceRoot, "packages/modules");
  const resolvedVirtualId = `\0${options.virtualId}`;
  const discover = () =>
    readdirSync(modulesDirectory, { withFileTypes: true })
      .filter((entry) => entry.isDirectory())
      .flatMap((entry) => {
        const packageJson = resolve(modulesDirectory, entry.name, "package.json");
        if (!existsSync(packageJson)) return [];
        const manifest = JSON.parse(readFileSync(packageJson, "utf8")) as ModulePackage;
          ...
          : [
              {
                ...
                modulePath: resolve(modulesDirectory, entry.name, modulePath),
              },
            ];
      })
  ...
  return {
    ...
    buildStart(this: { addWatchFile: (path: string) => void }) {
      this.addWatchFile(modulesDirectory);
    ...
    handleHotUpdate<Module>(context: {
      ...
      if (!context.file.startsWith(`${modulesDirectory}/`)) return;
    ...
    load(id: string) {
      ...
      return `${imports}\nconst discovered = [${values}]${filter};\nconst ids = new Map();\nfor (const entry of discovered) {\n  const id = entry.value.moduleId ?? entry.value.id;\n  const previous = ids.get(id);\n  if (previous !== undefined) throw new Error(\`Duplicate module id \${id}: \${previous}, \${entry.source}\`);\n  ids.set(id, entry.source);\n}\nexport default discovered.map((entry) => entry.value);`;
    },
  } satisfies Plugin;
};
...
export const moduleSettings = (workspaceRoot?: string, include?: ReadonlySet<string>) =>
  workspaceModules({
    name: "macrograph-module-settings",
    virtualId: "virtual:macrograph-module-settings",
...
/** Discovers standalone engine deployments and engine-less module exports. */
export const moduleDeployments = (workspaceRoot?: string, include?: ReadonlySet<string>) =>
  workspaceModules({
    name: "macrograph-module-deployments",
    virtualId: "virtual:macrograph-module-deployments",
    ...(workspaceRoot === undefined ? {} : { workspaceRoot }),
    exportName: "default",
    modulePath: (manifest) =>
