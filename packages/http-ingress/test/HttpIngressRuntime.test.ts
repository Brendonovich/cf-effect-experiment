...
const TestPlugin = Plugin.make({ id: "test", engine: TestEngine, effect: () => Effect.void });
...
const TestIngress = HttpIngress.make({
  ...
  configuration: Schema.Struct({ token: Schema.String, topics: Schema.Array(Schema.String) }),
  event: TestEvent,
  mergeConfiguration: (current, next) => ({
    token: current.token,
    topics: [...new Set([...current.topics, ...next.topics])],
  }),
  accepts: (configuration, eventType) => configuration.topics.includes(eventType),
});

it.effect("resolves, mounts, dispatches, and removes HTTP ingress handlers", () =>
  Effect.gen(function* () {
          ...
          handle: (request) =>
            Effect.succeed(
              request.configuration.token === "token"
                ? {
                    status: 200,
                    events: [{ event: TestEvent.make({ value: request.endpoint.metadata.value }) }],
                  }
                : { status: 403 },
            ),
        }),
    ...
    const deployment = Engine.withHttpIngress(
      Engine.deployment(TestPlugin, TestEngine.toLayer(() => Effect.die("Test engine is not hosted"))),
      {
        handlers: [live],
        requirements: (state) =>
          Effect.succeed(
    ...
    const endpoints = HttpEndpoint.Host.of({
      ...
      get: (handler, instanceKey) => {
        ...
        return Schema.decodeUnknownEffect(handler.metadata)(existing.metadata).pipe(
    ...
    const runtime = yield* HttpIngressRuntime.make([deployment], registry, endpoints);
    const manifest = yield* runtime.resolveManifest({ test: { values: ["one", "two"] } });
    ...
    yield* runtime.reconcile(manifest, []);
    assert.deepStrictEqual(operations, ["mount:one", "mount:two", "remove:one", "remove:two"]);
  }),
);
