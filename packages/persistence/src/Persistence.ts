  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
          ...
          ProjectMutation.$match(mutation, {
            ...
            SaveConnection: ({ graphId, connection }) => {
              ...
              const connections = existing
                ...
                : [...graph.connections, connection];
              const next = new Map(cached);
              next.set(graphId, { ...graph, connections });
              return next;
            },
            DeleteConnection: ({ graphId, connectionId }) => {
              const graph = cached.get(graphId);
      ...
      const updateCaches = (mutation: ProjectMutation) =>
        Effect.all([Ref.update(cache, updateCachedProject(mutation)), updateGraphCache(mutation)]);

      return Service.of({
        saveProject: (project) =>
          persistence.saveProject(project).pipe(Effect.tap(() => setProjectCache(project))),
        ...
        saveGraph: (graph) =>
          persistence
            .saveGraph(graph)
            .pipe(Effect.tap(() => updateCaches(ProjectMutation.SaveGraph({ graph })))),

        deleteGraph: (graphId) =>
          persistence
            .deleteGraph(graphId)
            .pipe(Effect.tap(() => updateCaches(ProjectMutation.DeleteGraph({ graphId })))),

        saveNode: (graphId, node) =>
          persistence
            .saveNode(graphId, node)
            .pipe(Effect.tap(() => updateCaches(ProjectMutation.SaveNode({ graphId, node })))),

        deleteNode: (graphId, nodeId) =>
          persistence
            .deleteNode(graphId, nodeId)
            .pipe(Effect.tap(() => updateCaches(ProjectMutation.DeleteNode({ graphId, nodeId })))),
        ...
        saveConnection: (graphId, connection) =>
          persistence
            ...
            .pipe(
              Effect.tap(() =>
                updateCaches(ProjectMutation.SaveConnection({ graphId, connection })),
              ),
            ),
        ...
        deleteConnection: (graphId, connectionId) =>
          persistence
            ...
            .pipe(
              Effect.tap(() =>
                updateCaches(ProjectMutation.DeleteConnection({ graphId, connectionId })),
              ),
            ),
      });
    }),
  ...
  Effect.gen(function* () {
    ...
    return Service.of({
      ...
      saveConnection: (graphId, connection) =>
        Ref.update(
          cache,
          updateCachedProject(ProjectMutation.SaveConnection({ graphId, connection })),
        ),

      deleteConnection: (graphId, connectionId) =>
        Ref.update(
          cache,
          updateCachedProject(ProjectMutation.DeleteConnection({ graphId, connectionId })),
        ),
    });
  }),
);
