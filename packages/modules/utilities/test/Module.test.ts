import type { Registration } from "@macrograph/module";

import { assert, describe, it } from "@effect/vitest";
import { Registration as ModuleRegistration } from "@macrograph/module";
import { Effect, Logger, Redacted, Result } from "effect";

import { TickEvent } from "../src/Definition.ts";
import UtilitiesModule, { formatValue, parseFormatString } from "../src/Module.ts";

const schemas = ModuleRegistration.collect(UtilitiesModule.effect);
...
describe("Utilities module", () => {
  it.effect("registers six complete schemas without duplicating Logic's Branch", () =>
    Effect.gen(function* () {
      const registered = yield* schemas;
      assert.deepStrictEqual(
        registered.map((item) => item.id),
        ["Print", "ConcatStrings", "IntToString", "FormatString", "FormatTime", "Tick"],
      );
      ...
      assert.deepStrictEqual(
        ...
        [{ id: "tick", name: undefined, type: "Int" }],
      );
    }),
  );
