...
Six schemas: Print, Concat Strings, Int To String, Format String,
Format Time, and Tick.

Branch lives in the Logic module.
...
- With Duration enabled, milliseconds are elapsed time and Day.js duration
  ...
  Day.js component semantics. An empty format uses the duration module's default.
...
does not read the clock. The Day.js duration and UTC modules are bundled dependencies.
...
Run tests with `pnpm --filter @macrograph/module-utilities run test --run`.
