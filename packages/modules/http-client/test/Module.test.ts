...
import HttpClientModule from "../src/Module.ts";
...
describe("HTTP client module", () => {
  it.effect("registers seven schemas preserving existing IDs, pins, and defaults", () =>
    Effect.gen(function* () {
      const schemas = yield* Registration.collect(HttpClientModule.effect);
      ...
      Effect.gen(function* () {
        ...
        const executor = yield* Executor.make(project, {
          engineClient: (moduleId) =>
            Effect.succeed(moduleId === HttpClientModule.id ? runtime : {}),
        });
        yield* executor.module(TestModule, TestDeployment);
        yield* executor.module(HttpClientModule, HttpClientDeployment);
        yield* executor.handleEvent(TestModule, new Start());
        ...
        const failed = yield* Effect.result(executor.handleEvent(TestModule, new Start()));
