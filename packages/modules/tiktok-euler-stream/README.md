...
guarantee is supplied by the module. Keys are only sent to this fixed WSS endpoint;
...
module. Removing a key does not erase older backups or snapshots; revoke it with
...
structures; use the JSON module for these rather than expecting struct/map pins.
...
Exports: `.` is the default module; `/Definition` contains `TikTokEngine`, schemas,
...
`/Events` exports `decodeEvent`. Existing module and node/schema IDs are unchanged.

The host must add the workspace dependency and TypeScript reference, register the
module/deployment for server execution, and discover the settings export. The
...
Run `pnpm --filter @macrograph/module-tiktok-euler-stream test run` and
`pnpm --filter @macrograph/module-tiktok-euler-stream typecheck`. Tests inject fake clients to
