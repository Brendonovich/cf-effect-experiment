  ...
  effect: Effect.fnUntraced(function* (context) {
    for (const [suffix, type] of [
      ...
      yield* context.schema.register({
        ...
        type: "pure",
        io: (io) => ({
          input: io.data.in("input", type, { name: "Number", defaultValue: 0 }),
          divisor: io.data.in("divisor", type, { name: "Divisor", defaultValue: 1 }),
          output: io.data.out("remainder", type, { name: "Remainder" }),
        }),
        run: ({ io }) =>
          Effect.gen(function* () {
            if (io.divisor === 0)
    ...
    for (const [id, name, inputType, calculate] of [
      ...
      yield* context.schema.register({
        ...
        type: "pure",
        io: (io) => ({
          one: io.data.in("one", inputType, {
            ...(id === "ExponentFloats" ? { name: "Number" } : {}),
            defaultValue: 0,
          }),
          two: io.data.in("two", inputType, {
    ...
    for (const [id, name, type] of [
      ...
      yield* context.schema.register({
        ...
        io: (io) => ({
          min: io.data.in("min", type, { name: "Min", defaultValue: 0 }),
          max: io.data.in("max", type, { name: "Max", defaultValue: 1 }),
          output: io.data.out("output", type),
        }),
        run: ({ io }) =>
          Effect.gen(function* () {
