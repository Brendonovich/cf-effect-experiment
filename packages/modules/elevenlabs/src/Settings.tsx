...
export interface SettingsProps {
  ...
  readonly rpc: {
    readonly ElevenLabsUpdateKey: (payload: {
      readonly apiKey: string;
    }) => Effect.Effect<void, unknown>;
    readonly ElevenLabsClearKey: () => Effect.Effect<void, unknown>;
  };
...
const Settings: Component<SettingsProps> = (props) => {
  ...
  const save = action(async function* (clear: boolean) {
    ...
    const success = await Effect.runPromise(
      clear ? props.rpc.ElevenLabsClearKey() : props.rpc.ElevenLabsUpdateKey({ apiKey }),
    ).then(
      () => true,
      () => false,
    );
  ...
  return (
    <section sx={styles.root}>
      ...
      <p sx={styles.note}>
        ...
        server and not returned by module settings. Protect project storage and backups.
      </p>
      <div sx={styles.row}>
        <input
...
export const settings = ClientSettings.make({
  ...
  renderInvalid: () => <p sx={styles.note}>Module settings state is unavailable.</p>,
});
