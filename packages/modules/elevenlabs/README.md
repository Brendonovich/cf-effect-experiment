...
Runtime-compatible adaptation of MacroGraph's ElevenLabs package. Import the module
from `@macrograph/module-elevenlabs`, its deployment from `/Deployment`, and editor
...
`filePathOut` output are intentionally not reproduced. This module never writes
files, plays audio, or streams execution scopes. A future native-capability module
...
`ElevenLabsClearKey`, neither of which returns the key. The module does not encrypt
...
Run `pnpm --filter @macrograph/module-elevenlabs exec vitest run` and `pnpm typecheck`
