...
const schemas = Registration.collect(JsonModule.effect);
...
describe("JSON module", () => {
    ...
    Effect.gen(function* () {
      ...
      for (const [type, value] of [
        ...
        assert.deepStrictEqual(
          (yield* run(schema(registered, "FromJSON"), { in: text }, { type }, definitions)).get(
            "out",
          ),
          Option.some(value),
        );
      ...
      assert.deepStrictEqual(
        (yield* run(
          schema(registered, "FromJSON"),
          { in: '{"_type":"wrong","date":"2026-09-06T00:00:00Z","names":[]}' },
          { type: custom },
          definitions,
        )).get("out"),
        Option.none(),
      ...
      assert.deepStrictEqual(
        (yield* run(
          schema(registered, "JSONGetScalarList"),
          { in: text },
          { type: custom },
          definitions,
        )).get("out"),
  ...
  it.effect("requires ToJSON's wildcard input and defaults other JSON-text inputs", () =>
    Effect.gen(function* () {
      const registered = yield* schemas;
      for (const [type, value] of [
        ["String", ""],
        ...
        for (const list of [false, true]) {
          ...
          assert.strictEqual(
            (yield* run(schema(registered, "ToJSON"), { in: list ? [] : value }, properties)).get(
              "out",
            ),
  ...
  it.effect("registers wildcard conversion IO without type or list properties", () =>
    Effect.gen(function* () {
      const registered = yield* schemas;
      ...
      for (const id of ["ToJSON", "FromJSON", "JSONGetScalarList"]) {
        ...
        assert.deepStrictEqual(item.properties, []);
      }
    }),
  );
