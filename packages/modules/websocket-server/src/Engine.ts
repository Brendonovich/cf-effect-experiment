...
const isIpv6 = (host: string) => {
  ...
  if (![...left, ...right].every((segment) => /^[\da-f]{1,4}$/i.test(segment))) return false;
...
export const make = Effect.fnUntraced(function* (
  ...
  const state = yield* SubscriptionRef.make<ReadonlyMap<ServerId, Entry>>(new Map());
  ...
  const validate = Effect.fnUntraced(function* (
    ...
    if (host.length === 0 || (!isIpv4(host) && !isIpv6(host) && !isHostname(host)))
      return yield* new InvalidServer({
    ...
    if (
      [...entries.values()].some(
        (entry) =>
          entry.definition.id !== ignoredId && addressesOverlap(entry.definition, normalized),
      )
    )
      return yield* new InvalidServer({
  ...
  const stopUnsafe = Effect.fnUntraced(function* (id: ServerId) {
    ...
    yield* disconnectClients(entry, "server", "Server stopped").pipe(
      Effect.ensuring(entry.fiber === undefined ? Effect.void : Fiber.interrupt(entry.fiber)),
    );
  });
  ...
  const startUnsafe = Effect.fnUntraced(function* (id: ServerId) {
    return yield* Effect.uninterruptibleMask((restore) =>
      Effect.gen(function* () {
        const current = yield* getEntry(id);
        if (Option.isNone(current)) return yield* new ServerNotFound({ id });
        if (current.value.status === "running" || current.value.status === "starting") return;

        const definition = current.value.definition;
        const activeConflict = [...(yield* SubscriptionRef.get(state)).values()].some(
          (entry) =>
          ...
          Effect.uninterruptibleMask((restoreClient) =>
            Effect.gen(function* () {
              const registered = yield* Effect.gen(function* () {
                const candidate = yield* SubscriptionRef.modify(state, (entries) => {
                  const entry = entries.get(id);
                  if (
                    entry === undefined ||
                  ...
                  let clientId = ClientId.make(globalThis.crypto.randomUUID());
                  while ([...entries.values()].some((candidate) => candidate.clients.has(clientId)))
                ...
                yield* updateEntry(id, (entry) => {
                  if (entry.generation !== generation || !entry.clients.has(candidate.value))
                    return entry;
              ...
              const clientExit = yield* Effect.exit(
                restoreClient(
                  client.run((message) =>
                    Effect.gen(function* () {
                      if (typeof message !== "string" || !message.isWellFormed()) return;
              ...
              const disconnected = Exit.isFailure(clientExit)
                ? {
                    ...
                    reason: Cause.hasInterrupts(clientExit.cause)
                      ...
                      : failureReason(clientExit.cause, "WebSocket peer failed"),
                  }
                : { cause: "peer" as const, reason: "Peer disconnected" };
              const removed = yield* SubscriptionRef.modify(state, (entries) => {
                const entry = entries.get(id);
                if (
                  entry === undefined ||
                  entry.generation !== generation ||
                ...
                return [true, new Map(entries).set(id, { ...entry, clients })];
              });
              if (removed)
                yield* mg.emit(
                  new ClientDisconnected({
        ...
        const lifecycle = Effect.scoped(
          ...
          Effect.flatMap((exit) => {
            const reason = Exit.isFailure(exit)
              ? failureReason(exit.cause, "WebSocket listener stopped unexpectedly")
        ...
        const listened = yield* Effect.result(
          restore(Deferred.await(ready)).pipe(Effect.onInterrupt(() => cleanupInterruptedStart)),
        );
        ...
        yield* updateEntry(id, (entry) =>
          entry.generation !== generation ? entry : { ...entry, status: "running" },
        );
      }),
    );
  });
  ...
  const checkedMessage = Effect.fnUntraced(function* (serverId: ServerId, message: string) {
    const size = utf8Size(message);
    ...
    Effect.raceFirst(
      client.send(message).pipe(
        Effect.timeoutOrElse({
          duration: "5 seconds",
          orElse: () =>
            Effect.fail(new SendFailed({ serverId, reason: "WebSocket send timed out" })),
        }),
    ...
    rpcs: RuntimeRpcs.toLayer({
      WebSocketServerSendToClient: ({ serverId, clientId, message }) =>
        Effect.gen(function* () {
          ...
          if (Option.isNone(current)) return yield* new ServerNotFound({ id: serverId });
          ...
          if (client === undefined) return yield* new ClientNotFound({ serverId, clientId });
      ...
      rpcs: ClientRpcs.toLayer({
        ...
        WebSocketServerUpdate: (input) =>
          Effect.gen(function* () {
            ...
            if (Option.isNone(current)) return yield* new ServerNotFound({ id: input.id });
...
export const localLayer = (adapter: Layer.Layer<Adapter>) => layer.pipe(Layer.provide(adapter));
