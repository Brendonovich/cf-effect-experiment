...
the module are inert: no socket is opened until a graph calls a light node.

## Configuration

Configure devices in module settings, using the bulb's MAC address and a reserved
...
configure this module: it can send LAN packets to configured addresses, and UDP
...
Six execution schemas are registered under module ID `lifx`:
...
Package exports: `.` (default module), `./Definition` (engine, resource, RPCs,
...
Register the same module in editor and executor catalogs, the deployment in the
...
Run `pnpm --filter @macrograph/module-lifx exec vitest run` and
`pnpm --filter @macrograph/module-lifx typecheck` after workspace installation.
