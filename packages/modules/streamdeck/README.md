...
This is not a Stream Deck SDK module. Configure the bridge to connect to the
...
commands exist in the legacy module.
...
`StreamDeck`-prefixed client RPCs. Settings bind to this module rather than the
generic server module. The protocol has no authentication; loopback binding is
recommended. No cloud deployment is provided.

Exports: module default, `Definition`, `Engine`, `Deployment`, and `Settings`
(named `settings`). Standalone discovery uses `./src/Deployment.ts`.

Runtime dependencies: `@macrograph/module`, `@macrograph/module-websocket-server`,
