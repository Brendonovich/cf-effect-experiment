...
Read-only nodes wrap transient `HelixError` failures in `@macrograph/module`'s `Retry`, preserving the original error as `cause`, including its status and rate-limit metadata. This covers Get Chat Settings, Get Channel Information, Get Streams, Get Users, Get Followers, and all GET actions in the table below (including Validate Token). Transport failures, HTTP 408, HTTP 429, and HTTP 5xx responses are transient. Invalid inputs, missing credentials/scopes, other HTTP errors, and malformed response data remain ordinary failures. Retry timing and attempt limits belong to the execution environment; nodes do not retry automatically. Write nodes do not grant retry permission.

These 16 read nodes also declare `replay: "safe"`; all other Twitch nodes use the default `"unsafe"`. This describes whether an unfinished invocation may be repeated after an unknown outcome, independently of its reported errors. The policy is carried to execution environments; workflow backend crash-recovery enforcement is still pending.
...
```sh
pnpm --filter @macrograph/module-twitch exec vitest run
pnpm --filter @macrograph/module-twitch typecheck
pnpm typecheck
```
