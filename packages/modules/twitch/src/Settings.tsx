...
import { ClientRpcs, ClientState, SUBSCRIPTION_TYPES, type AccountId } from "./Definition.ts";
...
const Settings: Component<SettingsProps> = (props) => {
  ...
  const isCollapsed = (accountId: AccountId) => collapsedAccounts().includes(accountId);
  ...
  const toggleConnection = (account: Account, connected: boolean, optimistic: () => void) =>
    void run(
      connected
    ...
    <section sx={styles.root}>
      ...
      <div>
        <For
          ...
          fallback={
            <div sx={styles.empty}>No Twitch credentials are available to this editor host.</div>
          }
        >
          {(account) => {
            const [state, setState] = createOptimistic(() => account().eventSubSocket.state);
            const [enabled, setEnabled] = createOptimistic(() => account().enabledSubscriptions);
            return (
              <section sx={styles.account}>
                ...
                >
                  <span sx={styles.chevronBox}>
                    <svg
                      ...
                      sx={styles.chevron}
                      style={{
                        transform: isCollapsed(account().id) ? "rotate(-90deg)" : undefined,
                      }}
                      aria-hidden="true"
                    >
                      <path
                  ...
                  <h3 sx={styles.accountName}>{account().displayName}</h3>
                </button>
                <span
                  sx={styles.stateDot}
                  style={{
                ...
                />
                <div sx={styles.stateLabel}>EventSub {socketStateLabel(state())}</div>
                <button
                  type="button"
                  ...
                  aria-busy={state() === "connecting" || isPending(state) ? "true" : undefined}
                  onClick={() => {
                        ...
                        return (
                          <div sx={styles.subscription}>
                            <span style={{ "min-width": "0" }}>
                              ...
                              <span sx={styles.subscriptionId}>{subscription}</span>
                            </span>
                            <button
                              type="button"
                              role="switch"
                              ...
                              style={{
                                "background-color": isEnabled() ? "#16a34a" : "var(--gray-5)",
                              }}
                              onClick={() => {
...
export const settings = ClientSettings.make({
  ...
  renderInvalid: () => <p sx={styles.invalid}>Module settings state is unavailable.</p>,
});
