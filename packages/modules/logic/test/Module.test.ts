...
import LogicModule from "../src/Module.ts";

const schemas = Registration.collect(LogicModule.effect);
...
describe("Logic module", () => {
  it.effect("declares Branch and Switch ports without an implicit continuation", () =>
    Effect.gen(function* () {
      const registered = yield* schemas;
      for (const [id, outputs, properties] of [
        ["Branch", ["true", "false"], {}],
        ["Switch", ["exec", "key-0", "key-1"], { number: 2 }],
      ] as const) {
        const item = schema(registered, id);
        assert.strictEqual(item.type, "base");
  ...
  it.effect("defaults concrete and Option inputs while requiring wildcard values", () =>
    Effect.gen(function* () {
      const registered = yield* schemas;
      for (const [type, value] of [
        ...
        assert.strictEqual(
          (yield* run(schema(registered, "UnwrapOptionOr"), { or: value }, { type })).outputs.get(
            "output",
          ),
          value,
        );
        ...
        for (const id of ["Cache", "Copy"])
          assert.strictEqual(
            (yield* run(schema(registered, id), { in: value }, { type })).outputs.get("out"),
            value,
          );
      }
    }),
  );
  it.effect("registers shared wildcards without type/list selectors or fake loop/scope nodes", () =>
    Effect.gen(function* () {
      const registered = yield* schemas;
  ...
  it.effect("captures and shallow-copies lists inferred through the whole-value wildcard", () =>
    Effect.gen(function* () {
      const registered = yield* schemas;
      for (const [type, values] of [
        ...
        for (const id of ["Cache", "Copy"]) {
          ...
          assert.deepStrictEqual((yield* run(item, { in: [] }, properties)).outputs.get("out"), []);
        }
