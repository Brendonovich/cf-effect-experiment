...
import { EngineTest } from "@macrograph/module";
...
describe("WebSocket client engine", () => {
  ...
  it.effect("persists definitions, exposes resources, and manages message lifecycle", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        assert.deepStrictEqual(
          yield* WebSocketConnection.values.pipe(Effect.provide(engine.resources)),
          [{ id, display: "Primary" }],
        );
  ...
  it.effect("validates URL fields without exposing credentials", () =>
    Effect.gen(function* () {
      const local = yield* makeHarness();
      yield* Effect.gen(function* () {
        const { client } = yield* EngineTest.makeClients(WebSocketClientEngine);
        for (const input of [
          ...
          if (Result.isFailure(result)) {
            assert.strictEqual(result.failure._tag, "WebSocketInvalidConnection");
      ...
      yield* Effect.gen(function* () {
        const { engine } = yield* EngineTest.makeClients(WebSocketClientEngine);
  ...
  it.effect("decodes persisted connections from before optional fields were added", () =>
    Effect.gen(function* () {
      const decoded = yield* Schema.decodeUnknownEffect(WebSocketClientEngine.Storage)({
        connections: [
          {
            id: "legacy",
            name: "Legacy",
  ...
  it.effect("drops callbacks from a socket after its definition is replaced", () =>
    Effect.gen(function* () {
  ...
  it.effect("cancels in-flight connections without allowing stale completion", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        const connecting = yield* client.WebSocketConnect({ id }).pipe(Effect.forkChild);
        yield* Deferred.await(harness.created);
        yield* Effect.yieldNow;
        yield* client.WebSocketDisconnect({ id });
        ...
        harness.sockets[0]?.open();
        yield* Effect.yieldNow;
        assert.strictEqual((yield* engine.client.state).connections[0]?.status, "disconnected");

        const reconnecting = yield* client.WebSocketConnect({ id }).pipe(Effect.forkChild);
        while (harness.sockets.length < 2) yield* Effect.yieldNow;
        yield* client.WebSocketRemoveConnection({ id });
        assert.isTrue(Result.isFailure(yield* Effect.result(Fiber.join(reconnecting))));
        harness.sockets[1]?.open();
        assert.deepStrictEqual(yield* engine.client.state, {
          connections: [],
        });
        ...
        const updating = yield* client.WebSocketConnect({ id: updateId }).pipe(Effect.forkChild);
        ...
        assert.isTrue(Result.isFailure(yield* Effect.result(Fiber.join(updating))));
        ...
        assert.isTrue(
          Result.isFailure(yield* Effect.result(client.WebSocketRemoveConnection({ id: missing }))),
        );
      }).pipe(Effect.provide(harness.engineLayer));
    }),
  );
  ...
  it.effect("reports connection, send, and size failures as typed errors", () =>
    Effect.gen(function* () {
      const harness = yield* makeHarness([], { autoOpen: false });
      yield* Effect.gen(function* () {
        const { client, runtime } = yield* EngineTest.makeClients(WebSocketClientEngine);
        const id = yield* client.WebSocketAddConnection({
          name: "Failures",
          url: "ws://localhost:8080",
        });
        ...
        if (Result.isFailure(failedConnection))
          assert.strictEqual(failedConnection.failure._tag, "WebSocketConnectionFailed");
        assert.isTrue(harness.storage().connections[0]?.connectOnStartup);
        ...
        if (Result.isFailure(notConnected))
          assert.strictEqual(notConnected.failure._tag, "WebSocketNotConnected");
        ...
        if (Result.isFailure(oversized))
          assert.strictEqual(oversized.failure._tag, "WebSocketMessageTooLarge");
      }).pipe(Effect.provide(harness.engineLayer));
  ...
  it.effect("connects startup definitions and closes their sockets with engine scope", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          const { engine } = yield* EngineTest.makeClients(WebSocketClientEngine);
          yield* Deferred.await(harness.created);
          while ((yield* engine.client.state).connections[0]?.status !== "connected") {
            yield* Effect.yieldNow;
          }
          assert.strictEqual(harness.sockets.length, 1);
        }).pipe(Effect.provide(harness.engineLayer)),
