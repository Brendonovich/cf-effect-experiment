...
import { ConnectionId, MessageReceived, WebSocketConnection } from "../src/Definition.ts";
import WebSocketClientModule from "../src/Module.ts";

describe("WebSocket client module", () => {
  it.effect("registers a resource-backed send action and filtered receive event", () =>
    Effect.gen(function* () {
      const schemas = yield* Registration.collect(WebSocketClientModule.effect);
      ...
      assert.isTrue(
        yield* eventSchema.matches(new MessageReceived({ connectionId: id, data: "hello" }), {
          connection: id,
        }),
      );
      assert.isFalse(
        yield* eventSchema.matches(new MessageReceived({ connectionId: other, data: "hello" }), {
          connection: id,
        }),
      );
