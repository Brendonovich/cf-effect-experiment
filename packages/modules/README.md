...
MacroGraph `packages/packages` catalog, alongside the modules already ported from
...
Stateless modules only export `Module.make({ id, effect })`; no engine definition,
...
registers it directly if it has no engine. Modules with engines still need an
explicit deployment for the host. Programmatic registration is `editor.module(module)`
and `executor.module(module)`, or `ModuleMount.register(executor, module)` for both.
...
container, `localhost` refers to the container. No new device-connected modules
...
OpenAI and ElevenLabs keys are configured in hosted module settings. Workflow
...
Integration credentials are configured through module settings and retained in
...
to trusted users; editors can configure modules and run graphs.
...
not an OAuth access token. VTube Studio requires local user approval of the module.
...
graphs. Module/schema identifiers and pin shapes follow the current API.
...
through the Filesystem module on an opted-in server.
...
- Math, String, Logic, and List cover runtime-compatible portions of the old
  ...
  execution-time random selection. Generic List, Logic/Option, Cache/Copy and JSON
  conversion nodes infer types from wildcard connections, including nested/custom
  values, rather than exposing Type/List properties.
