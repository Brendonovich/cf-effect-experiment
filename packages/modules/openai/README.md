...
Runtime-compatible adaptation of MacroGraph's OpenAI package. Import the module from
`@macrograph/module-openai`, its deployment from `/Deployment`, and editor settings
...
not encrypted by this module: secure the host's storage and transport accordingly.
...
Run `pnpm --filter @macrograph/module-openai exec vitest run` and `pnpm typecheck` after
