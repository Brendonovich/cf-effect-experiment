...
unchanged power, brightness and temperature. As in the source module, requested changes apply
...
Package: `@macrograph/module-elgato-key-light`. Default root export is the module catalog.
...
`HttpClient` service. `/Deployment` default exports `Engine.deployment(module, layer)` and is declared
in `macrograph.standaloneDeployment`. `/Settings` exports the `settings` descriptor.

The application must install the workspace dependency, register the module and settings,
...
Run `pnpm --filter @macrograph/module-elgato-key-light exec vitest run` and
`pnpm --filter @macrograph/module-elgato-key-light typecheck` after workspace installation.
